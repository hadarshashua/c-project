...
int System::logIn(string& userName, string& passWord, int& index)//	THE LOGIN FUNCTION WILL RETURN A POINTER TO SELLER OR BUYER WHO IS USING CURRENTLY THE SYSTEM 
{
...
int System::searchForProduct(Order** orderCart, int size, const string& productName, int& orderIndex)//Search the product at the sellers products array
{
	...
	for (i = 0; i < size; i++)
	{
		...
		{
			if (orderCart[i]->getProductsArray().getArrByIndex(i).getName() == productName)
			{
				orderIndex = i;
				return j;
			}
